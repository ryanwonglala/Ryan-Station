  ...
  const translations = {
    en: {
      ...
      musicPlayer: {
        ...
        settings: 'Configure DeepSeek API Key',
        ...
        djKeyPrompt: 'Enter DeepSeek API Key — only needed if the station key is not configured (leave blank to clear):',
        djKeySaved: '> SYSTEM: DeepSeek API key saved.',
        djKeyCleared: '> SYSTEM: DeepSeek API key cleared.',
        djNoKey: '> SYSTEM: DJ booth is offline — no station key configured. Click the gear to plug in your own DeepSeek key (platform.deepseek.com).',
        djError: '> SYSTEM: Signal lost — {error}',
    ...
    zh: {
      ...
      musicPlayer: {
        ...
        djKeyPrompt: '输入 DeepSeek API Key——仅在站点未配置公用 key 时需要（留空则清除）：',
        djKeySaved: '> 系统：DeepSeek API Key 已保存。',
        djKeyCleared: '> 系统：DeepSeek API Key 已清除。',
        djNoKey: '> 系统：DJ 台离线——站点未配置公用 key。点齿轮可填入你自己的 DeepSeek key（platform.deepseek.com）。',
        djError: '> 系统：信号丢失——{error}',
